...
export class FilmsService {
    ...
    init(): void {
        const file: Buffer = fs.readFileSync(path.resolve(__dirname, '../data/data.csv'));
        ...
        for (let i = 2; i < this.lineStarts.length; i++) {
            const string = file.slice(this.lineStarts[i - 1] || 0, this.lineStarts[i])
        ...
        if (query.year || query.genre) {
            if (query.year) {
                ...
                if (!query.genre) {
                    ...
                    resultItemIndexes = filteredYear.filter(i => this.genreIndex[query.genre][i])
                }
            } else if (query.genre) {
                resultItemIndexes = Object.keys(this.genreIndex[query.genre]).map(i => Number(i)) || []
            }


            resultItemIndexes = resultItemIndexes.slice(query.pageNumber * query.pageSize, (query.pageNumber + 1) * query.pageSize)
        } else {
            resultItemIndexes = Array.from(Array(query.pageSize), (v, index) => query.pageNumber * query.pageSize + index + 2)
        }

        let fd: fs.promises.FileHandle | undefined;
        try {
            const fd = await fs.promises.open(path.resolve(__dirname, '../data/data.csv'))

            return Promise.all(resultItemIndexes.map(async index => {
    ...
    async fsRead(fd: number, from: number, size: number): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            fs.read(fd, Buffer.allocUnsafe(1000), 0, size, from, (err, bytes, data) => {
